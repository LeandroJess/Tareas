...
SET _JRE_CANDIDATE=
IF "%JRE%" == "" IF EXIST "%APPDATA%\Google\AndroidStudio2022.1\studio64.exe.jdk" (
  SET /P _JRE_CANDIDATE=<"%APPDATA%\Google\AndroidStudio2022.1\studio64.exe.jdk"
)
IF "%JRE%" == "" (
...
IF "%VM_OPTIONS_FILE%" == "" (
  ...
  ) ELSE IF EXIST "%APPDATA%\Google\AndroidStudio2022.1\studio64.exe.vmoptions" (
    SET "USER_VM_OPTIONS_FILE=%APPDATA%\Google\AndroidStudio2022.1\studio64.exe.vmoptions"
  )
)
...
"%JAVA_EXE%" ^
  ...
  -Djava.system.class.loader=com.intellij.util.lang.PathClassLoader -Didea.strict.classpath=true -Didea.vendor.name=Google -Didea.paths.selector=AndroidStudio2022.1 -Didea.platform.prefix=AndroidStudio -Didea.jre.check=true -Dsplash=true ^
